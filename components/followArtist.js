      ...
      <h2 className="font-bold text-2xl -mb-5">Trending Artist on Tribals</h2>
        <div className='carouselz2Container w-full grid grid-flow-col auto-cols-[55%] gap-3 overflow-x-auto overscroll-x-contain snap-x snap-mandatory snap-start scroll-px-2 scroll-smooth py-20 relative md:auto-cols-[33%] lg:auto-cols-[19%]'>
            ...
            {cardArtist.map((card, index) => (
                ...
                <div className="w-full flex justify-between md:px-2 py-3">
                    <div className="grid grid-col text-xs lg:text-base">
                        <span className='font-semibold whitespace-nowrap'>{card.name}</span>
                        <span className='italic font-semibold text-gray-500 whitespace-nowrap'>{card.location}</span>
                    </div>
                    <div className="border-1 w-20 h-8 rounded-full flex justify-center items-center bg-black text-white ml-2">
